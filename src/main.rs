...
impl TrafficScape {
    pub fn new(target_delta_time: f64) -> Result<Self, TrafficScapeNewError> {
    ...
    pub fn run_loop<
        ...
        RenderFn: FnMut(f64, &mut Canvas<OpenGl>),
    >(
        mut self,
        ...
        let (event_loop, windowed_context, canvas, ms_per_update) = (
            &mut self.event_loop,
            ...
            match &event {
                ...
                Event::RedrawRequested(_) => {
                    ...
                    if let Some(fps) = timestep.frame_rate() {
                        update_title(windowed_context.window(), fps);
                    }
    ...
    fn create_windowed_context(
        start_title: &str,
        event_loop: &EventLoop<()>,
    ) -> Result<ContextWrapper<PossiblyCurrent, Window>, TrafficScapeNewError> {
        // Set up the window
...
fn main() {
    ...
    let traffic_scape = TrafficScape::new(1.0 / 60.0).expect("failed to init trafficscape");

    // Start the simulation loop
    traffic_scape.run_loop(|_| {}, |_| {}, |_, canvas| {});
